...
class SearcherApp(QMainWindow):
    def __init__(self, font_name: str):
        ...
        self.existing_articles_cache = set()  # 🔹 Кэш существующих артикулов
    ...
    def _on_load_success(self, df):
        self.df = df
        self._build_cache_from_df(df)  # 🔹 Строим кэш после загрузки
    ...
    def _set_status(self, text, color):
        ...
        self.lbl_status.setStyleSheet(f"color: {color}; font-size: 13px; font-family: '{self.font_name}'; padding: 4px 0;")
    ...
    def _build_cache_from_df(self, df):
        """🔹 Создаёт/обновляет кэш артикулов из DataFrame"""
